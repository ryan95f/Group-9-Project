...
def write_to_django_database(nodes, node, parent_node=None):
    ...
    if nodes.text_node_id == node.get_id():
        text_node = TextNode.objects.create(parent=parent_node, position=node.position, text=node.content)

        # Save to Django database.
        text_node.save()
    else:
        book_node = BookNode.objects.create(parent=parent_node, position=node.position, node_type=node.get_id())
        ...
        for child in node.children:
            write_to_django_database(nodes=nodes, node=child, parent_node=book_node)
