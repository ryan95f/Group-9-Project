class NodeBank(object):
    ...
    def add_class(self, node_class):
        """Add the given node class to our node list."""
        self.node_classes[node_class.get_id()] = node_class

    def get_class(self, node_id):
        """Return the node class with the given node ID."""
    ...
    def set_text_node_id(self, node_id):
        """Set the node used for plain-text."""
        node = self.get_class(node_id)
        if node is not None:
    ...
    def set_argument_node_id(self, node_id):
        """Set the node used for arguments."""
        node = self.get_class(node_id)
        if node is not None:
    ...
    def get_by_attr(self, node_attr):
        """Return all the node classes which contain an attribute of the given name."""
        found_nodes = []
