...
\begin{document}
	...
	The proposed project that has been put forward to us, is to update and complete implementation of the current CAMEL system. The existing codebase is required to add extra functionality, which includes lexical comparisons, as well as documentation and unit tests to allow further development to CAMEL.

	To address the clients proposal, CAMEL 2.0 will issues a code standard. This will be PEP8 Python standard and will be the first priority to implement as well as add comments. During this process, exception handling will be added as well as the additional lines of code to prevent current bugs. Addressed functionality (see  functional requirements) will be implemented in sprints, the length of the sprint cycle for functionality is dependant on the complexity. For example, lexical comparisons may take longer than adding exception handling. Tests will be added prior to writing the code, though will be written in Django's test framework first. Additional or 'concept' functionality will be addressed last, to ensure high code quality. The documentation will be another continuous process for the developer. Though for the administrator / Lecturer the majority will be written once most of the functionality has come together.             	
	\section{Assumptions}
	Due to the fixed nature of the current system, there are some aspects that we will need to assume. Firstly, we are assuming that the functionality we are developing for is single threaded. This assumption is being made as the time scale to add threaded functionality is non-existent, and we are developing the system on Django's development server, which cannot simulate a multi-threaded environment. A further assumption we are making is that there will be no additional requirements added when implementation occurs, and these requirements are final. Given the time scale we have to make the system, it would be unlikely that any additional requirements could be completed to high quality. Any additional functionality will be dropped due to development scope. Finally, we are allowed to integrate third party solutions to parts of the system that are out of our range with the development time available. Third party applications include:
	\begin{itemize}
