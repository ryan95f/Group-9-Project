...
PDF_ROOT  = getattr(settings, 'PDF_ROOT')
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):     
        ...
        for module_code in args:
            ...
            # needed so that \documentclass{../../../texmf/camel} works
            os.chdir( os.path.dirname(tex_file) )

            # set pdf directory
            pdf_dir = os.path.join(PDF_ROOT, module_code)

            # construct file names
            base_name = os.path.splitext(tex_file)[0]
            raw_name = os.path.split(base_name)[1]
            pdf_file = os.path.join(raw_name + '.pdf')
            print pdf_file
...
#------------------------------------------------
# texify
def texify(tex_file, screen=False, cymraeg=False, answers=False, blanks=False):
